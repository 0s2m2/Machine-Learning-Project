...
threshold = 0.6
y_pred = knn_predict_with_rejection(knn, X_val, threshold)

# Evaluate only confident predictions
mask = y_pred != -1
coverage = np.mean(mask)
print(f"Coverage (fraction of confident predictions): {coverage:.2f}")
print("KNN Accuracy (confident predictions only):", accuracy_score(y_val[mask], y_pred[mask]))
...
joblib.dump(
    ...
    os.path.join(MODELS_DIR, "knn_pipeline.pkl")
)
